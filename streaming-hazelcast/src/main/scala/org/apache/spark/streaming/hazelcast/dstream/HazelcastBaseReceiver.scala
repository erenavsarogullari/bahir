...
private[dstream] trait HazelcastBaseReceiver {
  ...
  @transient
  protected lazy val distributedObject = SparkHazelcastService.getDistributedObject(props)

  protected var listenerRegistrationId = ""

  protected def start() {
    listenerRegistrationId = registerListener(distributedObject)
  }

  protected def stop() = unregisterListener(distributedObject, listenerRegistrationId)
